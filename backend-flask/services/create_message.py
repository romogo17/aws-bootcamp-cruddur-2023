...
class CreateMessage:
    ...
    def run(mode, message, cognito_user_id, message_group_uuid=None, user_receiver_handle=None):
        with tracer.start_as_current_span("create-message-run"):
            ...
            if model["errors"]:
                ...
                model["data"] = {"display_name": "Andrew Brown", "handle": cognito_user_id, "message": message}
            else:
                ...
                users = db.query_array_json(
                    sql, {"cognito_user_id": cognito_user_id, "user_receiver_handle": rev_handle}
                )
                print("USERS =-=-=-=-==")
                print(users)
